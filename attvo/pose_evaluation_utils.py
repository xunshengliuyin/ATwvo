...
def mat2euler(M, cy_thresh=None, seq='zyx'):
    ...
    r11, r12, r13, r21, r22, r23, r31, r32, r33 = M.flat # 返回重组后的一维数组下标为  的元素
    # cy: sqrt((cos(y)*cos(z))**2 + (cos(x)*cos(y))**2)
    cy = math.sqrt(r33*r33 + r23*r23)
    if seq=='zyx':
        if cy > cy_thresh: # cos(y) not close to zero, standard form
            z = math.atan2(-r12,  r11) # atan2(cos(y)*sin(z), cos(y)*cos(z))返回给定的 X 及 Y 坐标值的反正切值
    ...
    if x:
            ...
            Ms.append(np.array(
                            [[1, 0, 0],
                             ...
                             [0, sinx, cosx]]))
    if Ms:
            return functools.reduce(np.dot, Ms[::-1])#3ge33
    return np.eye(3)
...
def pose_vec_to_mat(vec):
    ...
    Tmat = np.concatenate((rot, trans), axis=1)#34
    hfiller = np.array([0, 0, 0, 1]).reshape((1,4))
    Tmat = np.concatenate((Tmat, hfiller), axis=0)#44
    return Tmat

def write_relative_pose(out_file, curr_pose, next_pose):
    """======================================
    # 计算相对位姿
    # T(t->s) = dot[inv[T(t->0)], T(s->0)]
    #         = dot[T(0->t), inv[T(0->s)]]
    # ====================================="""
    with open(out_file, 'w') as f:
        curr_pose = pose_vec_to_mat(curr_pose)
        next_pose = pose_vec_to_mat(next_pose)
        # 得到T(curr->next)
        # rel_pose = np.dot(next_pose, np.linalg.inv(curr_pose))
        # 得到T(next->curr)
        rel_pose = np.dot(curr_pose, np.linalg.inv(next_pose))
        tx = rel_pose[0, 3],
        ty = rel_pose[1, 3]
        tz = rel_pose[2, 3]
        rot = rel_pose[:3, :3]
